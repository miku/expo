...
// bms are examples for benchmarks
var bms = []struct {
	s string
	r rune
}{
...
func BenchmarkFindContains(b *testing.B) {
	for _, bm := range bms {
		name := fmt.Sprintf("len-%d", len(bm.s))
		b.Run(name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				FindContains(bm.s, bm.r)
			}
		})
	}
}
