// instead of parsing float, parse an int instead; extra: map with capacity
//
// real    0m34.962s
// user    3m40.744s
// sys     0m10.685s
...
func aggregate(rat io.ReaderAt, offset, length int, resultC chan map[string]*Measurements, sem chan bool, wg *sync.WaitGroup) {
	...
	fmt.Printf(".")
	var (
		data    = make(map[string]*Measurements, 1024) // add initial capacity
