...
var (
	n          = flag.Int("n", 1_000_000_000, "number of measurements to write")
	seed       = flag.Int("s", 21, "random seed")
	cpuprofile = flag.String("cpuprofile", "", "write cpu profile to file")
	numWorkers = flag.Int("w", runtime.NumCPU(), "number of workers")
	batchSize  = flag.Int("b", 1048576, "batch size")
)
...
func main() {
	...
	for i := 0; i < *numWorkers; i++ {
		wg.Add(1)
		go worker(numC, resultC, &wg)
	}
	go func(resultC chan string, done chan bool) {
...
// worker takes the number of requested measurements from a channel and will
// create a blob containing measurements.
func worker(numC chan int, resultC chan string, wg *sync.WaitGroup) {
	...
	for n := range numC {
		...
		for i := 0; i < n; i++ {
			...
			b.WriteString(";")
			b.WriteString(strconv.FormatFloat(m.temp, 'f', 1, 64))
			b.WriteString("\n")
		}
		resultC <- b.String()
	}
...
func randMeasurement() Measurement {
	i := rand.IntN(len(stations))
	m := rand.NormFloat64() + stations[i].temp
	return Measurement{name: stations[i].name, temp: m}
}
